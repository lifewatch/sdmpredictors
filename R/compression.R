#' Compress a file
#' 
#' The internal method \code{compress_file} compresses a file and returns the path to the result.
#' 
#' @usage
#' compress_file(filename, outputdir, method = "gzip", overwrite = FALSE, remove = TRUE)
#' 
...
#' @param method character. The default value is \code{"gzip"}, 
#' the supported methods are \code{"gzip"} and \code{"bzip2"}.
#' @param overwrite logical. If \code{TRUE} and the output file already exists then 
#' the file is silently overwritten, otherwise an exception is thrown.
#' @param remove logical. If \code{TRUE} then the input file is remove afterwards, 
#' otherwise not.
#' 
...
#' Decompress a file
#'
#' The internal method \code{decompress_file} decompresses a file and returns the path to the result.
#' 
#' @usage
...
#' @param outputdir character. Path to the output directory.
#' @param overwrite logical. If \code{TRUE} and the output file already exists then the 
#' file is silently overwritten, otherwise an exception is thrown.
#' @param remove logical. If \code{TRUE} then the input file is removed afterwards, 
#' otherwise not.
#' 
