...
`layer_citations` returns layer citations as text or as
...
- layers:
  ...
  character vector. Code of the layers from past, current and future
  climate layers. When no layers are provided (default), then all
...
Note that for some layers multiple references are returned as some of
the predictors have been published separately.

## See also

`layer_citations`, [`bibentry`](https://rdrr.io/r/utils/bibentry.html),
