...
- terrestrial:
  ...
  logical. When `TRUE`, then datasets that only have terrestrial data
  (seamasked) are returned.

- marine:

  logical. When `TRUE`, then datasets that only have marine data
...
By default it returns all datasets, when both `marine`, `freshwater` and
`terrestrial` are `FALSE` then only datasets without land- nor seamasks
are returned.
